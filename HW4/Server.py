    ...
    try:
        while True:
            ...
            if type==5:
                if subtype==0:
                    serversForClient=""
                    for server in myServerConnections.keys():
                        serversForClient+=str(server)+'\0'
                    serversForClient = serversForClient[:-1]
                    outGoingData2=struct.pack('>bbhh',5,0,len(serversForClient),0)
                    conn_socket.send(outGoingData2)
                    conn_socket.send(serversForClient.encode())
                if subtype==1:
                    conn_socket.send(b'pong')
                if subtype==2:
                    nameToRmv=conn_socket.recv(6).decode()
                    if nameToRmv in clients:
